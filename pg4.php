<?php 
	...
	$sql = "select a.*,f.nome as filmeParticipou from tblator as a, tblfilme as f where a.participacao = f.id and a.destaque=1";
	
	$select = mysql_query($sql);

	$rs = mysql_fetch_array($select);
	
	$participacao = $rs['filmeParticipou'];
	
	if($participacao == null){
		
		$sql = "select * from tblator where destaque=1";
		
		$select = mysql_query($sql);

		$rs = mysql_fetch_array($select);
		
		$participacao = "Nenhum Filme";
			...
			<div id="produtosDetalhes">	
				...
				<div id="conteudoProdutosDetalhes">
					...
					<div class="detalhesProdutos">
						...
						<p class="negrito">Data de Nascimento: <?php echo($dataNasc);?></p>
						<p class="negrito">Participação no Filme: <?php echo($participacao);?></p>
					</div>					
				</div>
